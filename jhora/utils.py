...
"""
_DEBUG_APP = False
if _DEBUG_APP: print("JHora: Entering utils")
# Add parent directory to sys.path
import os, sys
sys.path.append(os.path.dirname(os.path.dirname(os.path.abspath(__file__))))
try:
    if _DEBUG_APP: print("JHora: utisl: Importing codes to dateutils")
    import csv
    import json
    from datetime import datetime,timedelta
    from dateutil import relativedelta
except Exception as e:
    print("JHora: utils Import in utils failed",e)
try:
    if _DEBUG_APP: print("JHora: utils: importing jhora.const")
    from jhora import const
    if _DEBUG_APP: print("JHora: utils: importing jhora.const success")
except Exception as e:
    print("JHora: utils: importing const failed",e)
if _DEBUG_APP: print("JHora: utils: calling const.get_swe")
swe = const.get_swe()
if _DEBUG_APP: print("JHora: utils: Importing jhora.panchanga.drik")
...
def use_database_for_world_cities(enable_database=False):
    ...
    if enable_database:
        print("Opening ",const._world_city_csv_file)
...
def _get_place_from_ipinfo():
    ...
    try:
        if _DEBUG_APP: print("JHora utils importing requests")
        import requests
        if _DEBUG_APP: print("JHora utils importing requests successful")
    except Exception as e:
        print("JHora utils importing requests failed",e)
...
def get_place_from_user_ip_address():
    ...
    try:
        try:
            if _DEBUG_APP: print("JHora: importing geocoder")
            import geocoder
            if _DEBUG_APP: print("JHora: importing geocoder successful")
            g = geocoder.ip('me') #ipinfo('me')
        except Exception as e:
            print("JHora: importing geocoder failed",e)
        if _DEBUG_APP: print("JHora: Trying to get using IP Address of the user")
        #print('g',g,g.city,g.country,g.latlng)
        if g==None or g=='':
            print('JHora: Trying using ipinfo website')
            try:
                place,latitude,longitude,time_zone_offset = _get_place_from_ipinfo()
                return place,latitude,longitude,time_zone_offset
            except Exception as e:
                error_msg = f"JHora: No latitude/longitude provided. Could not guess location from IP Address\n"+\
                            "Fill data manually."
                print(error_msg)
                #log_error(error_msg)
            ...
            time_zone_offset = get_place_timezone_offset(latitude,longitude)
            print('JHora: Location obtained from IP Address:',place,[latitude,longitude,time_zone_offset])
            return place,latitude,longitude,time_zone_offset
    except Exception as e:
        error_msg = f"JHora: No latitude/longitude provided. Could not guess location from IP Address\n"+\
                    "Fill data manually."
        ...
        #show_error_popup(error_msg)
        return []
""" This function is not used and hence commented
...
def _validate_data(place,latitude,longitude,time_zone_offset,dob,tob,division_chart_factor):
    ...
    if tob==None:
        tob = tuple(str(datetime.now()).split()[1].split(':'))
    ...
    if division_chart_factor not in const.division_chart_factors:
        ...
        try:
            if _DEBUG_APP: print("JHora utils importing warnings")
            import warnings
            if _DEBUG_APP: print("JHora utils importing warnings successful")
        except Exception as e:
            print("JHora utils importing warnings failed",e)
        warnings.warn(w_msg)
...
def get_location_using_nominatim(place_with_country_code):
    ...
    """
    try:
        if _DEBUG_APP: print("JHora: importing geopy.geocoders.Nominatim")
        from geopy.geocoders import Nominatim
        if _DEBUG_APP: print("JHora: importing geopy.geocoders.Nominatim successful")
        geolocator = Nominatim(user_agent="Astro") #,format_string="%s, Bangalore")
    except Exception as e:
        print("JHora: importing geopy.geocoders.Nominatim failed",e)
...
def _get_timezone_from_pytz(timezone_str_from_geocoder):
    ...
    try:
        if _DEBUG_APP: print("JHora utils importing pytz.timezone")
        from pytz import timezone
        if _DEBUG_APP: print("JHora utils importing pytz.timezone successful")
    except Exception as e:
        print("JHora utils importing pytz.timezone failed",e)
    
    tz = datetime.now(timezone(timezone_str_from_geocoder)).utcoffset().total_seconds()/60/60
    return tz
def get_place_timezone_offset(latitude, longitude):
    ...
    try:
        try:
            if _DEBUG_APP: print("JHora: utils importing timezonefinder utc")
            from pytz import utc, timezone
            from timezonefinder import TimezoneFinder
            if _DEBUG_APP: print("JHora: utils importing timezonefinder utc successful")
        except Exception as e:
            print("JHora: utils importing timezonefinder utc failed",e)
        tf = TimezoneFinder()
        today = datetime.now()
...
def _read_resource_messages_from_file(message_file):
    ...
    cal_key_list = {}
    try:
        if _DEBUG_APP: print("JHora: utils importing codecs")
        import codecs
        if _DEBUG_APP: print("JHora utils importing codecs successful")
    except Exception as e:
        print("JHora utils importing codex failed",e)
...
def newton_polynomial(x_data, y_data, x):
    ...
    def _poly_newton_coefficient(x, y):
        ...
        Returns: list of Newton polynomial coefficients
        """
        m = len(x)
        x = x[:]  # shallow copy
        a = y[:]  # shallow copy
...
def previous_panchanga_day(panchanga_date,minus_days=1):
    np_date = datetime(*panchanga_date)
    prev_date = np_date - timedelta(days=minus_days)
    p_date_str = prev_date.strftime("%Y-%m-%d").split('-')
    if len(p_date_str) == 4:
...
def next_panchanga_day(panchanga_date,add_days=1):
    ...
    prev_date = np_date + timedelta(days=int(add_days_int))
    p_date_str = prev_date.strftime("%Y-%m-%d").split('-')
...
def panchanga_date_diff(panchanga_date1,panchanga_date2):
    npdate1 = datetime(*panchanga_date1) ; npdate2 = datetime(*panchanga_date2)
    days_diff = (npdate2-npdate1)/timedelta(days=1)
...
def panchanga_time_delta(panchanga_date1, panchanga_date2):#,**kwargs=None):
    np_date1 = datetime(*panchanga_date1)
    np_date2 = datetime(*panchanga_date2)
    diff_days = (np_date1-np_date2)/timedelta(days=1)
    return diff_days
...
def get_dob_years_months_60hrs_from_today(dob,tob):
    ...
    current_date_str,_ = datetime.now().strftime('%Y,%m,%d;%H:%M:%S').split(';')
...
def vaakya_tamil_month(year, month_number):
    ...
    diff=kddays-1826555
    month_birthday = datetime(1900,1,1,0,0,0)+datetime.timedelta(days=diff)
    ...
    next_mon_birthday = datetime(1900,1,1,0,0,0)+datetime.timedelta(days=next_mon_kd-1826555)
    num_days_in_this_month = (next_mon_birthday-month_birthday).days
    
    return tamil_month_names[month_number-1], datetime.strftime(month_birthday, '%d-%m-%Y'), weekday, num_days_in_this_month, month_start_kd
...
def convert_h_to_p_to_planet_names(chart_1d):
    global PLANET_NAMES, resource_strings
    result = []
    for x in chart_1d:
        if x == 'L':
            result.append(resource_strings['ascendant_str'])
        elif '/' in x:
            parts = x.split('/')
            if all(p.isdigit() for p in parts):
                planet_names = [PLANET_NAMES[int(p)] for p in parts]
                result.append('\n'.join(planet_names))
            else:
                result.append('')
        elif x.isdigit():
            result.append(PLANET_NAMES[int(x)])
        else:
            result.append('')
    return result
...
get_key_from_list_value = lambda d, item: next((k for k, v in d.items() if item in v), None)
if _DEBUG_APP: print("JHora: Leaving utils")
