...
def _dhasa_duration(planet_positions,sign,varsha_narayana=False):
    ...
    if varsha_narayana:
        dhasa_period *= 3
    return dhasa_period
def _narayana_dhasa_calculation(planet_positions,dhasa_seed_sign,dob,tob,place,ayanamsa_mode=None,years=1, months=1, sixty_hours=1,include_antardhasa=True,varsha_narayana=False):
    ...
    dhasa_start_jd = drik.next_solar_date(jd_at_dob, place,ayanamsa_mode=ayanamsa_mode, years=years, months=months, sixty_hours=sixty_hours)
...
def narayana_dhasa_for_divisional_chart(dob,tob,place,ayanamsa_mode=None,years=1, months=1, sixty_hours=1,divisional_chart_factor=1,include_antardhasa=True):
    if ayanamsa_mode is None: ayanamsa_mode=const._DEFAULT_AYANAMSA_MODE
    if divisional_chart_factor==1:
        return narayana_dhasa_for_rasi_chart(dob, tob, place,ayanamsa_mode=ayanamsa_mode, years=years, months=months, 
                                             sixty_hours=sixty_hours, include_antardhasa=include_antardhasa)
    ...
    """
    # Get Varga Chart
    varga_planet_positions = charts.divisional_chart(jd_at_dob, place, ayanamsa_mode=ayanamsa_mode, divisional_chart_factor=divisional_chart_factor)
    ...
    return _narayana_dhasa_calculation(varga_planet_positions,dhasa_seed_sign,dob,tob,place,ayanamsa_mode=ayanamsa_mode,years=years, months=months, sixty_hours=sixty_hours,include_antardhasa=include_antardhasa,varsha_narayana=False)
def narayana_dhasa_for_rasi_chart(dob,tob,place,ayanamsa_mode=None,years=1,months=1,sixty_hours=1,include_antardhasa=True):
    if ayanamsa_mode is None: ayanamsa_mode=const._DEFAULT_AYANAMSA_MODE
    jd_at_dob = utils.julian_day_number(dob, tob)
    planet_positions = charts.rasi_chart(jd_at_dob, place, ayanamsa_mode=ayanamsa_mode)
    ...
    return _narayana_dhasa_calculation(planet_positions,dhasa_seed_sign,dob,tob,place,ayanamsa_mode=ayanamsa_mode,years=years,months=months,sixty_hours=sixty_hours,include_antardhasa=include_antardhasa,varsha_narayana=False)
...
def varsha_narayana_dhasa_bhukthi(dob,tob,place,ayanamsa_mode=None,years=1,months=1,sixty_hours=1,divisional_chart_factor=1,include_antardhasa=True):
    if ayanamsa_mode is None: ayanamsa_mode = const._DEFAULT_AYANAMSA_MODE
    jd_at_dob = utils.julian_day_number(dob, tob)
    jd_at_years = drik.next_solar_date(jd_at_dob, place,ayanamsa_mode=ayanamsa_mode, years=years)
    rasi_planet_positions = charts.rasi_chart(jd_at_years, place,ayanamsa_mode=ayanamsa_mode)
    p_to_h_rasi = utils.get_planet_house_dictionary_from_planet_positions(rasi_planet_positions)
    varga_planet_positions = charts.divisional_chart(jd_at_years, place, ayanamsa_mode=ayanamsa_mode,divisional_chart_factor=divisional_chart_factor)
    p_to_h_varga = utils.get_planet_house_dictionary_from_planet_positions(varga_planet_positions)
    ...
    nd = _narayana_dhasa_calculation(varga_planet_positions, dhasa_seed_sign, dob,tob,place,ayanamsa_mode=ayanamsa_mode,years=years,months=months,sixty_hours=sixty_hours,include_antardhasa=include_antardhasa,varsha_narayana=True)
    return nd
