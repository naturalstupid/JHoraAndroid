...
def _dhasa_seed(jd,place,lagna_house,lagna_lord_house):
    ...
    if birth_time > dawn_start and birth_time < dawn_end: # dawn
        ...
        _dhasa_seed = lagna_house    
    return _dhasa_seed
def get_dhasa_antardhasa(dob,tob,place,ayanamsa_mode=None,divisional_chart_factor=1,years=1,months=1,sixty_hours=1,include_antardhasa=False):
    if ayanamsa_mode is None: ayanamsa_mode=const._DEFAULT_AYANAMSA_MODE
    jd_at_dob = utils.julian_day_number(dob, tob)
    jd_years = drik.next_solar_date(jd_at_dob, place, ayanamsa_mode=ayanamsa_mode,years=years, months=months,sixty_hours=months)
    from jhora.horoscope.chart import charts, house
    pp = charts.divisional_chart(jd_years, place, ayanamsa_mode=ayanamsa_mode,divisional_chart_factor=divisional_chart_factor)
