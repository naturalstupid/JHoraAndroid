...
def lagna_kendradhi_rasi_dhasa(dob,tob,place,ayanamsa_mode=None,divisional_chart_factor=1):
    if ayanamsa_mode is None: ayanamsa_mode=const._DEFAULT_AYANAMSA_MODE
    return kendradhi_rasi_dhasa(dob,tob,place,ayanamsa_mode=ayanamsa_mode,divisional_chart_factor=divisional_chart_factor)
def kendradhi_rasi_dhasa(dob,tob,place,ayanamsa_mode=None,divisional_chart_factor=1,include_antardhasa=True):
    if ayanamsa_mode is None: ayanamsa_mode=const._DEFAULT_AYANAMSA_MODE
    jd_at_dob = utils.julian_day_number(dob, tob)
    planet_positions = charts.divisional_chart(jd_at_dob, place,ayanamsa_mode=ayanamsa_mode, divisional_chart_factor=divisional_chart_factor)
    ...
    for c,dhasa_lord in enumerate(dhasa_progression):
        ...
        if total_dhasa_duration >= const.human_life_span_for_narayana_dhasa:
            break
    return dhasa_info
def karaka_kendradhi_rasi_dhasa(dob,tob,place,ayanamsa_mode=None,divisional_chart_factor=1,karaka_index=1,include_antardhasa=True):
    if ayanamsa_mode is None: ayanamsa_mode=const._DEFAULT_AYANAMSA_MODE
