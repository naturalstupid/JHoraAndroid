...
def vimsottari_dasha_start_date(jd,place,ayanamsa_mode=None):
    ...
    _yoga = drik.yogam(jd, place,ayanamsa_mode=ayanamsa_mode)
...
def vimsottari_mahadasa(jdut1,place,ayanamsa_mode=None):
    """List all mahadashas and their start dates"""
    if ayanamsa_mode is None: ayanamsa_mode = const._DEFAULT_AYANAMSA_MODE
    lord, start_date = vimsottari_dasha_start_date(jdut1,place,ayanamsa_mode=ayanamsa_mode)
    retval = Dict()
    for i in range(9):
...
def get_dhasa_bhukthi(jd,place,ayanamsa_mode=None,use_tribhagi_variation=False,antardhasa_option=1):
    """
    ...
    _,_,_,tz = place
    dashas = vimsottari_mahadasa(jd,place,ayanamsa_mode=ayanamsa_mode)#V4.2.9
