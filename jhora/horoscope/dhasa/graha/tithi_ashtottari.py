...
def ashtottari_adhipathi(tithi_index):
    for key,(tithi_list,durn) in ashtottari_adhipathi_dict.items():
        if tithi_index in tithi_list:
            return key,durn 
def ashtottari_dasha_start_date(jd,place,tithi_index=1,ayanamsa_mode=None):
    if ayanamsa_mode is None: ayanamsa_mode=const._DEFAULT_AYANAMSA_MODE
    _,_,_,birth_time_hrs = utils.jd_to_gregorian(jd)
    tit = drik.tithi(jd, place,ayanamsa_mode=ayanamsa_mode,tithi_index=tithi_index)
...
def ashtottari_mahadasa(jd,place,tithi_index=1,ayanamsa_mode=None):
    """
    ...
    """
    if ayanamsa_mode is None: ayanamsa_mode=const._DEFAULT_AYANAMSA_MODE
    lord, start_date = ashtottari_dasha_start_date(jd,place,tithi_index,ayanamsa_mode=ayanamsa_mode)
    retval = Dict()
...
def ashtottari_anthara(dhasa_lord, bhukthi_lord,bhukthi_lord_start_date):
    ...
    for _ in range(len(ashtottari_adhipathi_list)):
        ...
        lord = ashtottari_next_adhipati(lord)
    return retval
def get_ashtottari_dhasa_bhukthi(jd, place,ayanamsa_mode=None,use_tribhagi_variation=False,include_antardhasa=True,
    ...
    dashas = ashtottari_mahadasa(jd,place,tithi_index,ayanamsa_mode=ayanamsa_mode)
    dhasa_bhukthi=[]
    for i in dashas:
        dhasa_lord = i
