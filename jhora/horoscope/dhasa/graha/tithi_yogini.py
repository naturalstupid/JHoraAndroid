...
def _antardhasa(dhasa_lord,antardhasa_option=1):
    ...
    return _bhukthis
def _dhasa_start(jd,place,tithi_index=1,ayanamsa_mode=None):
    if ayanamsa_mode is None: ayanamsa_mode=const._DEFAULT_AYANAMSA_MODE
    _,_,_,birth_time_hrs = utils.jd_to_gregorian(jd)
    tit = drik.tithi(jd, place,tithi_index=tithi_index,ayanamsa_mode=ayanamsa_mode)
...
def get_dhasa_bhukthi(dob,tob,place,ayanamsa_mode=None,include_antardhasa=True,use_tribhagi_variation=False,tithi_index=1,
                      antardhasa_option=1):
    """
    ...
    dhasa_lord, start_jd,_ = _dhasa_start(jd,place,tithi_index=tithi_index,ayanamsa_mode=ayanamsa_mode)
    retval = []
