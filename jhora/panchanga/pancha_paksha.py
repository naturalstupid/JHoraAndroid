...
def get_matching_pancha_pakshi_data_from_db(bird_index, weekday_index, paksha_index):
    result_list = []
    with open(PP_DB_FILE, encoding='utf-8') as f:
        reader = csv.reader(f)
        for row in reader:
            try:
                if (int(row[_NAK_BIRD_INDEX]) == bird_index - 1 and
                    int(row[_WEEK_DAY_INDEX]) == weekday_index - 1 and
                    int(row[_PAKSHA_INDEX]) == paksha_index - 1):
                    result_list.append(row[:_LAST_COL_FOR_READING + 1])
...
def construct_pancha_pakshi_information(dob=None,tob=None,place=None,nakshathra_bird_index=None):
    ...
    for row in range(0,len(result_list),5):
        ...
        main_bird_image = const.get_resource_path(pancha_pakshi_images[int(mbi)])
        ...
        nkbi = const.get_resource_path(pancha_pakshi_images[int(mbi)])
        ppbi = const.get_resource_path(pancha_pakshi_images[int(ppi)]) 
        ...
        bpbi = const.get_resource_path(pancha_pakshi_images[int(bpi)])
        kp_icon = const.get_resource_path('shukla_paksha.png') if int(pi)==0 else  const.get_resource_path('krishna_paksha.png')
        ...
        for irow in range(row,row+5):
            ...
            sub_bird_image = const.get_resource_path(pancha_pakshi_images[int(sbi)])
            sub_act = utils.resource_strings[pancha_pakshi_activities[int(sai)]+'_str']
            sub_act_image = const.get_resource_path(pancha_pakshi_activity_images[int(sai)])
